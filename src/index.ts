        ...
        if (op === "cmd") {
            if (cmd === "onConnect") {
                ...
                if (opt.heartbeatUrl) {
                    ...
                    }, (this.connectionInfo && this.connectionInfo.heartbeatIntervalMs) || opt.heartbeatIntervalMs || 10000) as any
                }
...
export class JsonServiceClient {
    ...
    apply(f:(client:JsonServiceClient) => void) {
        f(this)
        return this
    }
...
export function camelCaseAny(o:any) {
    if (!o || !(o instanceof Object) || Array.isArray(o)) return o
    let to: any = {}
    for (let k in o) {
...
export function sanitizeFormData(formData:FormData) {
    ...
    for (let [key, value] of formData) {
        ...
        if (typeof value == 'object' && (value as any).size === 0) {
            formData.delete(key)
        }
    }
    return formData
